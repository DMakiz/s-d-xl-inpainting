...
def predict(image, mask, prompt="", negative_prompt="", guidance_scale=7.5, steps=20, strength=1.0, scheduler="EulerDiscreteScheduler"):
    ...
    mask = np.array(mask)
    mask = np.invert(mask.astype(bool)).astype(np.uint8) * 255
    
    output = pipe(prompt=prompt, negative_prompt=negative_prompt, image=image, mask_image=mask, guidance_scale=guidance_scale, num_inference_steps=int(steps), strength=strength)
    
    inverted_mask = np.invert(mask)
    
    return inverted_mask, gr.update(visible=True)
...
with image_blocks as demo:
    ...
    with gr.Row():
                with gr.Column():
                    ...
                    mask = gr.Image(source='upload', tool='sketch', elem_id="mask_upload", type="numpy", label="Mask",height=400)
                ...
                with gr.Column():
                    mask_out = gr.Image(label="Inverted Mask", elem_id="mask_output", height=400)
                    with gr.Group(elem_id="share-btn-container", visible=False) as share_btn_container:
                        community_icon = gr.HTML(community_icon_html)
                        loading_icon = gr.HTML(loading_icon_html)
                        share_button = gr.Button("Share to community", elem_id="share-btn",visible=True)
    
    btn.click(fn=predict, inputs=[image, mask, prompt, negative_prompt, guidance_scale, steps, strength, scheduler], outputs=[mask_out, share_btn_container], api_name='run')
    prompt.submit(fn=predict, inputs=[image, mask, prompt, negative_prompt, guidance_scale, steps, strength, scheduler], outputs=[mask_out, share_btn_container])
